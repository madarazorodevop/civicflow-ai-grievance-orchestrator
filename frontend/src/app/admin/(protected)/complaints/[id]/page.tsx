...
export default function AdminComplaintDetail() {
  ...
  const approveComplaint = async (action: string, reason?: string) => {
    try {
      const res = await api.post(`/complaints/${id}/approve`, { action, reason });
      setC(res.data);
      toast.success(action === 'APPROVE' ? 'Complaint approved' : 'Complaint rejected');
    } catch {
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-sm mb-3">
              ...
              <span className={`px-2 py-0.5 rounded font-bold ${c.approvalStatus === 'APPROVED' ? 'bg-green-100 text-green-700' : c.approvalStatus === 'REJECTED' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                {c.approvalStatus || 'PENDING REVIEW'}
              </span>
            </div>
            
            {(!c.approvalStatus || c.approvalStatus === 'PENDING REVIEW') && (
              <div className="flex gap-2">
                <button onClick={() => approveComplaint('APPROVE')} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded font-bold text-sm">Approve</button>
                <button onClick={() => {
                  const r = prompt('Reason for rejection?');
                  if (r) approveComplaint('REJECT', r);
                }} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded font-bold text-sm">Reject</button>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-500">Risk Severity:</span><strong className={c.riskLevel === 'SEVERE' ? 'text-red-600' : 'text-slate-700 dark:text-slate-300'}>{c.riskLevel}</strong></div>
